...
from stocktrace.domain.entities.financial import FinancialStatement
...
def fpt_statements() -> list[FinancialStatement]:
    return [
        _make_statement(
            "FPT", "Q1", date(2025, 3, 31), Decimal("14000"), Decimal("2200"), Decimal("1000")
        ),
        _make_statement(
            "FPT", "Q2", date(2025, 6, 30), Decimal("15200"), Decimal("2400"), Decimal("1200")
        ),
        _make_statement(
            "FPT", "Q3", date(2025, 9, 30), Decimal("16500"), Decimal("2600"), Decimal("1500")
        ),
        _make_statement(
            "FPT", "Q4", date(2025, 12, 31), Decimal("17800"), Decimal("2900"), Decimal("1800")
        ),
    ]
...
def test_calculate_ratios_returns_one_per_period(
    ...
    assert len(ratios) == _QUARTER_COUNT


def test_roe_calculated(
    engine: FinancialRatioEngine, fpt_statements: list[FinancialStatement]
) -> None:
    ratios = engine.calculate(fpt_statements)
    latest = ratios[-1]
...
def test_revenue_growth_calculated(
    ...
    # Quarterly growth needs a comparable TTM window one year earlier; it must
    # not be presented as growth merely by comparing Q2 with Q1.
    assert ratios[1].revenue_growth is None
